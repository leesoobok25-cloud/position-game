...
매일 정해진 시간에 **여행 상황별 영어 회화 스크립트**를 텔레그램으로 보내주는 자동화입니다.
원어민 과외 전에 기초 회화를 미리 익히는 용도로 만들었습니다.

- **콘텐츠**: 공항·호텔·식당·길찾기·쇼핑 등 30개 여행 시나리오 (한 달 주기로 매일 하나씩 순환)
- **구성**: 각 스크립트는 `대화 → 핵심 표현 → 오늘의 팁`으로 되어 있어요.
...
### 보내는 시간 바꾸기
`.github/workflows/daily-english.yml` 의 `cron` 값을 수정하세요. **UTC 기준**이라 한국 시간 - 9시간입니다.
...
### 스크립트(시나리오) 추가·수정하기
`scenarios.py` 의 `SCENARIOS` 리스트에 같은 형식으로 항목을 추가하면 자동으로 순환에 포함됩니다.
...
| 파일 | 설명 |
|------|------|
| `scenarios.py` | 여행 영어 회화 스크립트 30개 (콘텐츠) |
| `send_daily.py` | 오늘의 스크립트를 골라 텔레그램으로 전송 |
| `../.github/workflows/daily-english.yml` | 매일 자동 실행하는 GitHub Actions 설정 |
