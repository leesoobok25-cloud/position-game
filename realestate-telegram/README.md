...
**서울·경기** 지역의 분양공고를 매일 텔레그램으로 받아보는 자동화입니다. 다루는 주택유형:

- 🏛🏙 **APT (아파트)** — 공공분양 + 민간분양 모두 (민간사전청약·신혼희망타운 포함)
...
- 분양공고는 공공분양이든 민간분양이든 모두 청약홈을 통해 접수되므로, API 하나로 **공공 + 민간을 함께** 받아볼 수 있어요.
...
- 이 알림은 **분양 전용 텔레그램 봇**을 따로 사용합니다 (영어 학습 봇과 별개, Secret 이름도 다름).
...
### 2단계. 분양 알림 전용 텔레그램 봇 만들기

1. 텔레그램에서 **@BotFather** 검색 → `/newbot` 입력 → 봇 이름과 사용자명(`...bot`으로 끝나야 함)을 정합니다 → 받은 **토큰**을 복사합니다.
2. 방금 만든 봇과 대화창을 열고 아무 메시지나 보냅니다(예: `hi`).
   - ⚠️ 이 단계를 안 하면 봇이 나에게 먼저 말을 걸 수 없습니다.
3. **@userinfobot** 검색 → `/start` → 알려주는 숫자 **ID**를 복사합니다.
   - (english-telegram을 설정할 때 확인한 내 ID와 같은 값입니다. 이미 알고 있다면 그대로 쓰면 됩니다.)
...
| `PRESALE_TELEGRAM_BOT_TOKEN` | 2단계에서 만든 **분양 알림 봇**의 토큰 |
| `PRESALE_TELEGRAM_CHAT_ID` | 2단계에서 확인한 숫자 ID |

> 영어 학습 봇의 `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID`와는 별개입니다. 이름이 다르니 헷갈리지 마세요.
...
- **내 컴퓨터에서:**
  ...
  APPLYHOME_API_KEY=내키 TELEGRAM_BOT_TOKEN=봇토큰 TELEGRAM_CHAT_ID=내아이디 python3 send_presale.py
  ```
...
### 받고 싶은 주택유형 조정
`applyhome_api.py`의 `ENDPOINTS` 목록에서 원하지 않는 줄을 지우면(예: 임의공급) 해당 유형은 더 이상 받지 않습니다.

### 공공/민간 분류 기준
`applyhome_api.py`의 `PUBLIC_HINTS` 목록(주택구분에 포함되면 공공분양으로 표시)에 단어를 추가/삭제해 조정할 수 있습니다. 분류가 애매할 수 있어 메시지에는 원본 구분값(예: 민영/국민 등)도 항상 함께 표시됩니다.
...
| 파일 | 설명 |
|------|------|
| `applyhome_api.py` | 청약홈 공공데이터 API 호출(주택유형별 5개 엔드포인트), 서울·경기 필터링, 공공/민간 분류 |
...
- **메시지가 안 와요**: ① `--preview`로 먼저 콘솔 출력이 되는지 확인하세요. ② Secret 이름이 정확한지(`APPLYHOME_API_KEY`, `PRESALE_TELEGRAM_BOT_TOKEN`, `PRESALE_TELEGRAM_CHAT_ID`) 확인하세요. ③ 새 봇에게 먼저 메시지를 보냈는지 확인하세요.
- **"인증키가 유효하지 않습니다" 같은 오류**: 공공데이터포털 활용신청이 아직 승인되지 않았거나, 인증키를 잘못 넣은 경우가 많습니다. 마이페이지에서 **Decoding** 값을 다시 복사해 보세요. (신청 직후에는 반영까지 1시간~하루 걸릴 수 있습니다.)
- **일부 유형만 안 와요**: 특정 엔드포인트 조회가 실패하면 그 유형만 건너뛰고 나머지는 정상 발송합니다. Actions 실행 로그에 "경고: ... 조회 실패" 메시지로 어떤 유형이 실패했는지 나옵니다.
- **신규 공고가 계속 0건이에요**: 정상입니다. 새로 올라온 서울·경기 공고가 없으면 조용히 넘어가고(메시지 발송 안 함) 로그에만 "신규 0건"으로 표시됩니다.
